...
def a_passing_case_leaves_nothing():
    """THE LEAK. `test/checker_parity_test.py:60,160` never removed its copy."""
...
def the_real_parity_suite_leaves_nothing():
    """The suite the audit named: it copytrees this whole repository, three times."""
    box = residue.workspace("parity-e2e")
    proc = subprocess.run([sys.executable, os.path.join(HERE, "checker_parity_test.py")],
                          capture_output=True, text=True, env=dict(os.environ, TMPDIR=box))
    assert proc.returncode == 0, proc.stdout[-2000:] + proc.stderr[-2000:]
    left = sorted(os.listdir(box))
    assert left == [], "checker_parity_test left %d entr(y|ies) in its TMPDIR: %s" % (len(left), left[:5])
    assert "this run left nothing" in proc.stdout, proc.stdout
...
for n, f in [
    ...
    ("checker_parity_test leaves an empty TMPDIR", the_real_parity_suite_leaves_nothing),
